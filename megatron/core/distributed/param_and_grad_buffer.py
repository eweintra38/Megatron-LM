...
# ─── MODE SWITCH ───────────────────────────────────────────────────────────────
# choose between "uniform" (IID) or "ge" (Gilbert-Elliott bursty)
DROP_MODEL = os.getenv("DROP_MODEL", "uniform").lower()
USE_GE = DROP_MODEL == "ge"
...
class _ParamAndGradBucketGroup:
    ...
    def __init__(
        ...
        # ─── 2. Optionally instantiate GE models ─────────────────────────
        if USE_GE:
            ...
            self._param_loss_model = None
            self._grad_loss_model  = None
    ...
    def start_param_sync(self, force_sync: bool = False):
        ...
        drop_prob = float(os.getenv("PACKET_LOSS_PROB_PARAM", "0.0"))
        #drop_prob = 0.3
        group = self.intra_distributed_optimizer_instance_group
        ...
        for bucket in self.buckets:
            ...
            for rank in range(world_size):
                ...
                if rank == local_rank:
                    ...
                    # bursty GE model
                    keep = not self._param_loss_model.should_drop()
                else:
                    # uniform IID
